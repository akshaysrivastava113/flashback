...
import Adder from "../create/Adder";
import Alter from "../create/Alter";
...
export default function UpdateQuestionaire(){
    ...
    useEffect(() => {
        axios.get(`${backend_url}/quest/${questIdParam}`, {
            headers: {
                 "Authorization":`Bearer ${signedIntoken}`
            }
        }).then((allSlides) => {
            console.log("allSlides", allSlides.data.slides);
            setQTitle(allSlides.data.title.title);
            setSlidesData(allSlides.data.slides);
        }).catch((err) => {
            console.log(err);
        }).finally(() => {
        })
    ...
    const updateQuest = ()=> {
        ...
        if(qTitle !== "" && slidesData.length !== 0){
            setLoading(true);
            const finalObj = {
                questionnaireId: questIdParam,
                questTitle: qTitle,
                slidesData: slidesData
            };
            console.log(finalObj);
            axios.patch(`${backend_url}/api/v1/edit`, finalObj, {
                headers: {
                    "Authorization":`Bearer ${signedIntoken}`
                }
            })
            ...
            <div id="questionaire-form" className="w-full lg:w-2/3 h-full flex flex-col justify-center items-center p-2 md:p-5 border-l border-r border-b border-gray-200">
                ...
                <div className="w-full grid grid-cols-3 grid-rows-2 m-5 p-5">
                    ...
                    <div className="flex justify-end items-center col-span-1">
                        <PrimaryButton customTailwind="p-2" execFunc={updateQuest} tailIcon={chatBubbleSign} text={loading?<BeatLoader color="#FFFFFF" size={5} />:'Update' } /> 
                    </div>

                    <div className="flex justify-start items-center col-span-2">
                        <input value={qTitle} onChange={(e) => {
