    ...
    return (
        <div id="signup-form-container" className="flex justify-center items-start border-2 h-screen">
            <div id="signup-form" className=" flex flex-col justify-center mt-32 m-5 p-5">
                ...
                <button onClick={() => sendSignupReq()} className={`border-2 h-12 mt-8 m-2 p-2 bg-red-400 text-white font-semibold ${loading?'opacity-20':'opacity-100'}`}>{loading?<BeatLoader color="#FFFFFF" size={5} />:'Sign up'}</button>
            </div>
        </div>
    )
}